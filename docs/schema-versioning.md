...
| `eval/results*.{json,md}` | evaluation evidence, not setup-bearing (D-8); the harness owns these files, so stamping never rewrites them. `eval/results-replay.*` is regenerated by `eval/run.py --replay`; the historical LIVE reports are preserved unaltered |
